...
const ROLES = [
  {
    ...
    color: T.yellow,
    icon: "👑",
    desc: "Acceso total al sistema. Puede crear y eliminar cualquier perfil.",
    perms: ["Dashboard", "Usuarios", "Asistencia", "Fitness", "Nutrición", "Planes", "Boletas", "Perfiles", "Administración"],
  },
  {
    id: "admin",
    label: "Administrador",
    color: T.info,
    icon: "🛡️",
    desc: "Gestión operativa completa. Sin acceso a configuración de perfiles.",
    perms: ["Dashboard", "Usuarios", "Asistencia", "Fitness", "Nutrición", "Planes", "Boletas"],
  },
  {
    id: "entrenador",
    label: "Entrenador",
    color: T.success,
    icon: "🏋️",
    desc: "Acceso a fitness, rutinas, nutrición y asistencia. Sin gestión financiera.",
    perms: ["Dashboard", "Usuarios (ver)", "Asistencia", "Fitness", "Nutrición"],
  },
  {
    id: "recepcion",
    label: "Recepción",
    color: "#A78BFA",
    icon: "🗂️",
    desc: "Control de asistencia y boletas. Sin acceso a datos de entrenamiento.",
    perms: ["Dashboard", "Asistencia", "Boletas"],
  },
];

/* ─── Mock system users ───────────────────────────── */
const INITIAL_PROFILES = [
  { id: 1, name: "Roberto Sánchez",  email: "super@solgym.com",    phone: "999-111-2222", role: "superadmin",  status: "Activo",   joined: "01/01/2024", lastLogin: "Hoy, 08:30" },
  { id: 2, name: "Diana Flores",     email: "admin@solgym.com",    phone: "999-333-4444", role: "admin",       status: "Activo",   joined: "15/03/2024", lastLogin: "Hoy, 07:45" },
  { id: 3, name: "Marco Quispe",     email: "coach1@solgym.com",   phone: "999-555-6666", role: "entrenador",  status: "Activo",   joined: "01/06/2024", lastLogin: "Ayer, 18:00" },
  { id: 4, name: "Sofía Mendoza",    email: "coach2@solgym.com",   phone: "999-777-8888", role: "entrenador",  status: "Activo",   joined: "10/08/2024", lastLogin: "hace 2 días" },
  { id: 5, name: "Camila Ríos",      email: "recep@solgym.com",    phone: "999-000-1111", role: "recepcion",   status: "Activo",   joined: "20/09/2024", lastLogin: "Hoy, 09:00" },
  { id: 6, name: "Jorge Palomino",   email: "coach3@solgym.com",   phone: "999-222-3333", role: "entrenador",  status: "Inactivo", joined: "05/02/2024", lastLogin: "hace 1 mes" },
];

const BLANK = { name: "", email: "", phone: "", role: "entrenador", status: "Activo" };
      ...
      <div style={{ ...card, width: "100%", maxWidth: "560px", maxHeight: "90vh", overflowY: "auto" }}>
        ...
        {tab === "datos" && (
          <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
            <Input label="Nombre completo" value={form.name}  onChange={set("name")}  placeholder="Ej: Marco Quispe" />
            ...
            <div style={{ background: `${role.color}11`, border: `1px solid ${role.color}33`, borderRadius: "10px", padding: "14px 16px" }}>
              ...
              <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                {role.perms.map(p => (
                  <span key={p} style={{ background: `${role.color}22`, color: role.color, fontSize: "10px", fontWeight: "700", padding: "2px 8px", borderRadius: "10px" }}>{p}</span>
                ))}
              </div>
            </div>
        ...
        {tab === "permisos" && (
          <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
            <p style={{ color: T.textSub, fontSize: "13px", margin: 0 }}>
              Los permisos se asignan automáticamente según el rol. Para personalizar, cambia el rol del perfil.
            </p>
            {ROLES.map(r => (
              <div key={r.id} style={{ background: form.role === r.id ? `${r.color}11` : T.dark3, border: `1px solid ${form.role === r.id ? r.color + "44" : T.border}`, borderRadius: "10px", padding: "14px 16px", cursor: "pointer", transition: "all 0.15s" }}
              ...
              >
                ...
                <p style={{ color: T.textMute, fontSize: "11px", margin: "0 0 8px" }}>{r.desc}</p>
        ...
        <div style={{ display: "flex", gap: "10px", marginTop: "22px" }}>
          ...
          <Btn onClick={() => { onSave(form); onClose(); }} disabled={!form.name || !form.email} full>
...
export default function Profiles() {
  ...
  const [modal,       setModal]       = useState(null);   // null | "new" | profile obj
  ...
  const saveProfile = form => {
    if (form.id) {
      setProfiles(prev => prev.map(p => p.id === form.id ? { ...p, ...form } : p));
    } else {
      setProfiles(prev => [...prev, { ...form, id: Date.now(), joined: new Date().toLocaleDateString("es-PE"), lastLogin: "Nunca" }]);
    }
  };

  const toggleStatus = id => {
    setProfiles(prev => prev.map(p => p.id === id ? { ...p, status: p.status === "Activo" ? "Inactivo" : "Activo" } : p));
  };

  const countByRole = id => profiles.filter(p => p.role === id).length;
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "12px" }}>
        <div>
          ...
          <p style={{ color: T.textSub, margin: "4px 0 0", fontSize: "13px" }}>Usuarios del sistema, roles y permisos de acceso</p>
        </div>
      ...
      <div style={{ display: "flex", gap: "12px", flexWrap: "wrap" }}>
        {ROLES.map(r => (
          <div key={r.id}
            ...
            style={{ ...card, flex: "1", minWidth: "130px", borderTop: `3px solid ${r.color}`, cursor: "pointer", opacity: filterRole !== "all" && filterRole !== r.id ? 0.5 : 1, transition: "opacity 0.15s" }}
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
              <span style={{ fontSize: "20px" }}>{r.icon}</span>
              <span style={{ color: r.color, fontSize: "28px", fontFamily: "Bebas Neue, sans-serif" }}>{countByRole(r.id)}</span>
            </div>
      ...
      <div style={{ ...card, padding: "14px 18px", display: "flex", gap: "12px", flexWrap: "wrap", alignItems: "center" }}>
        ...
        <span style={{ color: T.textSub, fontSize: "12px" }}>{filtered.length} perfiles</span>
      </div>
      ...
      <div style={{ ...card, padding: 0, overflowX: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", minWidth: "620px" }}>
          <thead>
            <tr style={{ borderBottom: `1px solid ${T.border}` }}>
              {["PERFIL", "ROL", "TELÉFONO", "ÚLTIMO ACCESO", "ESTADO", ""].map(h => (
          ...
          <tbody>
            {filtered.map(p => {
              ...
              return (
                <tr key={p.id}
                  style={{ borderBottom: `1px solid ${T.border}`, transition: "background 0.12s", cursor: "pointer" }}
                ...
                >
                  ...
                  <td style={{ padding: "13px 16px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                      <span>{r.icon}</span>
                      <Badge text={r.label} color={r.color} />
                    </div>
                  </td>
                  <td style={{ padding: "13px 16px", color: T.textSub, fontSize: "12px" }}>{p.phone}</td>
                  <td style={{ padding: "13px 16px", color: T.textMute, fontSize: "12px" }}>{p.lastLogin}</td>
                  <td style={{ padding: "13px 16px" }}>
                    <Badge text={p.status} color={p.status === "Activo" ? T.success : p.status === "Inactivo" ? T.textMute : T.danger} />
                  </td>
                  <td style={{ padding: "13px 16px" }}>
                    <div style={{ display: "flex", gap: "6px" }}>
                      <Btn size="sm" variant="ghost" onClick={e => { e.stopPropagation(); setModal(p); }}>Editar</Btn>
                      <Btn size="sm" variant="outline" onClick={e => { e.stopPropagation(); toggleStatus(p.id); }}>
                        {p.status === "Activo" ? "Desactivar" : "Activar"}
                      </Btn>
                      {p.role !== "superadmin" && (
                        <Btn size="sm" variant="danger" onClick={e => { e.stopPropagation(); setDelConfirm(p.id); }}>🗑</Btn>
                      )}
                    </div>
                  </td>
                </tr>
        ...
        {filtered.length === 0 && (
          <div style={{ padding: "40px", textAlign: "center", color: T.textMute, fontSize: "13px" }}>
      ...
      <div style={{ ...card }}>
        <h3 style={{ color: T.text, margin: "0 0 16px", fontSize: "14px", fontWeight: "700" }}>🔐 Referencia de permisos por rol</h3>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: "500px", fontSize: "12px" }}>
            <thead>
              <tr style={{ borderBottom: `1px solid ${T.border}` }}>
                <th style={{ color: T.textMute, textAlign: "left", padding: "8px 12px", fontWeight: "700", letterSpacing: "0.8px", fontSize: "10px" }}>SECCIÓN</th>
                {ROLES.map(r => (
                  <th key={r.id} style={{ color: r.color, textAlign: "center", padding: "8px 12px", fontWeight: "700", fontSize: "10px" }}>
                    {r.icon} {r.label}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {["Dashboard", "Usuarios", "Asistencia", "Fitness", "Nutrición", "Planes", "Boletas", "Perfiles", "Administración"].map(section => (
                <tr key={section} style={{ borderBottom: `1px solid ${T.border}` }}>
                  <td style={{ padding: "9px 12px", color: T.textSub }}>{section}</td>
                  {ROLES.map(r => {
                    const has = r.perms.some(p => p.toLowerCase().startsWith(section.toLowerCase()));
                    return (
                      <td key={r.id} style={{ padding: "9px 12px", textAlign: "center" }}>
                        {has
                          ? <span style={{ color: T.success, fontSize: "14px" }}>✓</span>
                          : <span style={{ color: T.dark4, fontSize: "14px" }}>—</span>
                        }
                      </td>
                    );
                  })}
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.88)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200 }}>
          <div style={{ ...card, maxWidth: "340px", textAlign: "center" }}>
            ...
            <div style={{ display: "flex", gap: "10px" }}>
              ...
              <Btn variant="danger" onClick={() => { setProfiles(p => p.filter(x => x.id !== delConfirm)); setDelConfirm(null); }} full>Eliminar</Btn>
            </div>
          </div>
        </div>
      )}
