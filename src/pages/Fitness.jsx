      ...
      <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", alignItems: "center" }}>
        ...
        {users.map(u => (
          <div key={u.id} onClick={() => onSelect(u.id)}
            style={{ display: "flex", alignItems: "center", gap: "7px", padding: "6px 12px", borderRadius: "8px", cursor: "pointer", background: userId === u.id ? T.yellowGlow : T.dark3, border: `1px solid ${userId === u.id ? T.yellow : T.border}` }}>
...
function MetricsTab({ users }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
      <UserSelector users={users} userId={userId} onSelect={(id) => { setUserId(id); setWeight(""); setHeight(""); setParts({}); }} />
...
function RoutinesTab({ users }) {
  ...
  const updateExercise = async (uid, field, val) => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
      <UserSelector users={users} userId={userId} onSelect={(id) => { setUserId(id); setRoutines({}); }} />
      <div style={{ display: "flex", gap: "16px", flexWrap: "wrap" }}>
                    ...
                    <div style={{ display: "flex", gap: "10px", alignItems: "center", flexWrap: "wrap" }}>
                      {[{ field: "sets", label: "Series" }, { field: "reps", label: "Reps" }, { field: "rest", label: "Seg" }].map(({ field, label }) => (
                        <div key={field} style={{ textAlign: "center" }}>
                          ...
                          <input type="number" value={ex[field] ?? ex.restSecs ?? 60}
...
export default function Fitness() {
  ...
  useEffect(() => {
    membersService.getAll().then(setUsers).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
      ...
      {tab === "rutinas"    && <RoutinesTab  users={users} />}
